...
import com.hzdongcheng.softwareplatform.controller.appmanager.proxyspublic.MobileWPublicProxy;
...
@Controller
@RequestMapping("/appesWPublicAction")
public class AppesWPublicController extends BaseController{
	private final static Logger log = Logger.getLogger(AppesWPublicController.class);
	private static Class proxyClass = null;
	private static MobileWPublicProxy mobileWPublicProxy = null;
	static {
		try {
 
			proxyClass = Class.forName("com.hzdongcheng.softwareplatform.controller.appmanager.proxyspublic.MobileWPublicProxy");
			mobileWPublicProxy = (MobileWPublicProxy) proxyClass.newInstance();
		} catch (Exception e) {}
	}
	@RequestMapping(value="/publicservice.do",  method = {RequestMethod.GET, RequestMethod.POST } ,produces = {"text/html;charset=UTF-8;"})
    @ResponseBody
	public String mobileservice(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String resultMsg = "";
	    log.info("AppesWPublicController mobileservice  request="  + message);
		try{
			...
			if(json != null){
				...
				log.error("[AppesWPublicController mobileservice   unpack error:],msg=" + message);
			}
		}catch(Exception e){
			log.error("[ AppesWPublicController mobileservice  handle request:],error= " + e.getMessage() + ",msg=" + message );
		}
		//返回处理结果
	...
	protected String handleRequest(JsonPacket packet,String message){
		...
		if(StringUtils.isNotEmpty(serviceName) && serviceName.length() > 10){
			String dtoName = "com.hzdongcheng.softwareplatform.intelligentstorage.bll.appPublicDto." + packet._ServiceName;
			...
			try{
                ...
                Object result = method.invoke(mobileWPublicProxy, new Object[]{inParam});
                //构造响应包
                responseMsg =  PacketUtils.buildSuccessResult(packet,result);
                if(responseMsg.length()>100 && packet._ServiceName.endsWith("Qry")){
                    log.info("AppesWPublicController handleRequest    _dcdz_response=>" + responseMsg.substring(0,100));
                }else{
                    log.info("AppesWPublicController handleRequest    _dcdz_response=>" + responseMsg);
                }
			} catch(java.lang.ClassNotFoundException e0){
				log.error("[AppesWPublicController handleRequest   ClassNotFoundException:],errmsg=" + e0.getMessage() + ",msg=" + message);
			} catch (NoSuchMethodException e1){
				log.error("[AppesWPublicController handleRequest   NoSuchMethodException:],errmsg=" + e1.getMessage() + ",msg=" + message);
                ...
                */
				...
				responseMsg = PacketUtils.buildFailResult(packet, errorMsg);
                log.error("[AppesWPublicController handleRequest   Buiness Error:],errmsg=" + errorMsg + ",msg=" + message);
				//if(apCfg.isLogRawMsg())
          		log.info("AppesWPublicController handleRequest   response=" + responseMsg);
            } catch(Exception e3){
				log.error("[AppesWPublicController handleRequest   Service Error:],errmsg=" + e3.getMessage() + ",msg=" + message);
			}
		}
		return responseMsg;
	}  
