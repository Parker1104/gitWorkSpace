...
<div id="showsearch" class="list" style="display: none">
    <form id="searchForm" action="${basePath}/userBlacklistController/user" method="post">
        <div class="list">
            <ul>
...
<div id="showadd" class="list" style="display: none">
    <form id="modifyCode" action="${basePath}/userBlacklistController/userSave.do" method="post" >
        <ul>
...
<div id="showupdate" class="list" style="display: none">
    <form id="modifyCodeupdate" action="${basePath}/userBlacklistController/userUpdate.do" method="post" >
        <ul>
...
function formToJson(id){//把form表单的所有数据变成json对象
    ...
    $(document).ready(function(){
        ...
        $("#datatable").jqGrid({
            url:'${basePath}/userBlacklistController/user',
            datatype: "json",
            mtype: "POST",
            ...
            editable: false,
            shrinkToFit:true,
            colNames:['用户卡号','用户姓名','类型','性别','证件类型','','状态','手机号','','','班级','邮箱','密码','','描述','有效期'],
            colModel:[
                {name:"usercardid",index:'usercardid',align:'left',width:70,sortable:false},
                {name:"username",index:"username",align:'center',width:70,sortable:false},
                ...
                {name:"subdepartment",index:'subdepartment',align:'center',width:110,sortable:false},
                {name:"address",index:'address',align:'center',width:110,sortable:false},
		...
		if(ids.length>1){
	    	...
	    	var dialogCode = dialog({
	            ...
	            ok: function () {
	                ...
	                $.ajax({
	                    type: "POST",
	                    url: "${basePath}/userBlacklistController/userDelete.do?id="+col.usercardid,
	                    success: function(msg){
	                    	showMsg(eval("("+msg+")").msg);
	                        $("#datatable").trigger('reloadGrid');
	                        setTimeout(function () {
