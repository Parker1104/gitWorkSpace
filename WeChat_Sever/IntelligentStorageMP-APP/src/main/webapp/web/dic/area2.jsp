				...
				/* if (treeNode) {
					...
					win = dialog({
						...
						ok : function() {
							...
							var areashortname = document
									.getElementById("areashortname").value;
							if (name == "" || name == null) {
		...
		function edit() {
			...
			if (treeNode) {
				...
				if (indexpos == -1) {
					...
					$("#areashortname").val(
							treeNode.name.substring(indexpos + 1, treeNode.name
									.indexOf(")")));
				}
				//var pattern =new RegExp("\\((.| )+?\\)","igm");
				//var shortname=treeNode.name.match(pattern);
				//$("#name").val(treeNode.name.replace(shortname,""));
				//$("#areashortname").val(shortname.replace(/\\(.*?\\)/g,""));
			}
		...
		<div id="" class="list" style="display: none;">
			...
			<form id="selectarea" action="">
				<ul
					...
					<li style="list-style-type: none; width: 425px; height: 50px;"><spring:message
							code="area.areacode" />：<input type="text" id="areaCode" name=""
						value="" /></li>
					<li style="list-style-type: none; width: 425px; height: 50px;"><spring:message
							code="area.areaname" />：<input type="text" id="areaName" name=""
						value="" /></li>
					<li style="list-style-type: none; width: 425px; height: 50px;"><spring:message
							code="area.areashortname" />：<input type="text"
						id="areaShortName" name="" value="" /></li>
