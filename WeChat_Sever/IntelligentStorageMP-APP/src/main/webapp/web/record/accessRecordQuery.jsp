...
<div class="content">
    <%-- <form id="searchMemberCardForm" action="${pageContext.request.contextPath}/accessRecord/list" method="post">
        <div class="list">
            <ul>
...
<div id="showsearch" class="list" style="display: none">
    <form id="searchMemberCardForm" action="${pageContext.request.contextPath}/accessRecord/list2" method="post">
        <div class="list">
            <ul>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#datatable").jqGrid({
            url:'${pageContext.request.contextPath}/accessRecord/list2',
            datatype: "json",
            mtype: "POST",
            ...
            //footerrow:true, //汇总
            shrinkToFit:true,
            colNames:['用户ID','终端号','箱编号','柜号','箱号','存物时间','用户类型','终端号','名称',/* '证件类型','证件号','有效日期', */'实收金额','找零金额','记录状态','取物时间','取件类型','操作'],
            colModel:[
                {name:"usercardid",index:"usercardid",align:'center',width:80,sortable:false},
                ...
                {name:"usertype",index:'usertype',align:'center',width:60,sortable:false,formatter:function(el,options,rowData){
                    if(el==0){return "普通用户";}else if(el==1){return "贵宾用户";}else{return "微信用户";}
                }},  
               ...
               /* {name:"idtype",index:"idtype",align:'center',width:60,sortable:false,formatter:function(el,options,rowData){
                ...
                {name:"",index:'',align:'center',width:50,sortable:false,formatter:function(el,options,rowData){
                	 //var u = '[<a href="javascript:void(0)" onclick="viewDetail(\''+ rowData.terminalid +'\',\''+rowData.boxid+'\',\''+rowData.usercardid+'\')">查看</a>]';
                	 var u= '<input type="button" value="查看" onclick="viewDetail(\''+ rowData.terminalid +'\',\''+rowData.boxid+'\',\''+rowData.usercardid+'\',\''+rowData.storeintime+'\',\''+rowData.taketime+'\')" style="margin-left:15px;" "/>';
    ...
    function viewDetail(id,ids,number,storeintime,taketime){
        ...
        if(viewGrid==null){
            viewGrid=showCommodity('detail','${pageContext.request.contextPath}/accessRecord/takeoutList?terminalid='+id+'&boxid='+ids+'&usercardid='+number+'&memo='+s1+'&cashierNo='+s2,800,function(data){
                //toLoadInput('loss_form',data.bill);
                return data.rows;
            }).showCol('validperiod').showCol('memo');
        }else{
            viewGrid.setGridParam({
                url:'${pageContext.request.contextPath}/accessRecord/takeoutList?terminalid='+id+'&&boxid='+ids+'&usercardid='+number+'&memo='+s1+'&cashierNo='+s2
            }).trigger('reloadGrid');
        }
        openWin('取物记录','showCommoditys',850);
    }
