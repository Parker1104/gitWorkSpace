...
<div id="showsearch" class="list" style="display: none">
    <form action="${basePath}/wristStrapHairpinStatisticsController/list" id="searchMemberCardForm" method="post">
			<div class="list">
				<ul>
 ...
 <div id ="foot" style="width:91%; margin-top: 10px;">
       <h5 id="h5height" style=" width: 105%;margin-top:0px; height: 30px; border: 1px solid #cdedff; background: #fff; border-top: none; line-height: 30px; text-align: left; float:left; margin-left: 2.5%; ">
            <span>合计</span>
            <span>总发卡数量：<label id="sendcardcount">${sendcount}</label></span>
            <span>总退卡数量：<label id="realmoney">${collectcount}</label></span>
            <span>总未退卡数量：<label id="balance">${notcollect}</label></span>
            <span>今日发卡数量：<label id="sendcardcount">${todaysendcount}</label></span>
            <span>今日退卡数量：<label id="realmoney">${todaycollectcount}</label></span>
            <span>今日未退卡数量：<label id="balance">${todaynotcollect}</label></span>
            <%-- <span>男用户数量：<label id="balance">${nan}</label></span>
            <span>女用户数量：<label id="balance">${nv}</label></span> --%>
       </h5>
</div>
...
function formToJson(id){//把form表单的所有数据变成json对象
    ...
    $(document).ready(function(){
        ...
        $("#datatable").jqGrid({
            url:'${basePath}/wristStrapHairpinStatisticsController/list',
            datatype: "json",
            mtype: "POST",
            ...
            //footerrow:true, 
            shrinkToFit:true,
            colNames:['收银员','收银终端号','时间','发卡数量','退卡数量','未退卡数量','备注'],  
            colModel:[
                ...
                {name:"sendcardcount",index:"sendcardcount",align:'center',width:200,sortable:false},
                {name:"collectcardcount",index:'collectcardcount',align:'center',width:200,sortable:false},
                {name:"notcollectcount",index:'notcollectcount',align:'center',width:200,sortable:false,formatter:function(el,options,rowData){
                    var a = rowData.sendcardcount-rowData.collectcardcount;
                    return a;
                }},
   ...
   var dialogCode=null;
   var selectMakeCard =  function(row){//弹出查询窗口
		   dialogCode = dialog({
	           title: '查询',
	           content: $('#showsearch'),
	           okValue: '查询',
