...
public class DateUtiles
{
     ...
     */  
    public static java.util.Date dateQu(java.util.Date dates)   {  
        SimpleDateFormat df = new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss");
        String datesString =df.format(dates);
        java.util.Date newDate=null;
		try {
			newDate = df.parse(datesString);
    ...
    public static void main(String[] args) {
 	   ...
 	   System.out.println("--1-------"+formatStr);
 	   
 	   
 	  java.util.Date  dates=dateQu(new java.util.Date()) ;
 	  SimpleDateFormat df = new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss");
 	  System.out.println("--2-------"+ df.format(dates) );
    ...
    public static void main(String args[])  throws Exception   {
		    ...
		    do{
            	...
            	if(compare(nowDate,storeintimeTemp) ){//一整天24小时的计算
           		     moneyTotal= moneyTotal+ cost( storeintime,  storeintimeTemp,   boxtypecode,   chargeModeInt);
	           	}else{//不到一天的计算
	           		 moneyTotal= moneyTotal+ cost( storeintimeTemp,  nowDate,   boxtypecode,   chargeModeInt);
	           		 break;
	           	}
